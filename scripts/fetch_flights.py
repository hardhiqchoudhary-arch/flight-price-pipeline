...
def get_airport_entity(airport_code):
    ...
    try:
        response = requests.get(url, headers=headers, params={"query": airport_code, "locale": "en-US"})
        data = response.json()
        ...
        if not results:
            ...
            return None, None
        for r in results:
            if r.get("skyId") == airport_code:
                print(f"Found: {r.get('presentation', {}).get('title')} (skyId={r.get('skyId')}, entityId={r.get('entityId')})")
                return r.get("skyId"), r.get("entityId")
        first = results[0]
        print(f"Found: {first.get('presentation', {}).get('title')} (skyId={first.get('skyId')}, entityId={first.get('entityId')})")
...
def fetch_flights(origin_sky, destination_sky, origin_entity, destination_entity, days_out):
    ...
    print(f"Fetching: {origin_sky} → {destination_sky} on {future_date}")
    querystring = {
    ...
    for flight in flights:
        try:
            ...
            """, (
                ...
                flight.get("duration"), flight.get("stops", 0)
            ))
            saved += 1
        except Exception as e:
...
if __name__ == "__main__":
    ...
    if not origin_sky or not destination_sky:
        print("Could not find airports. Check codes and try again.")
        exit()
    print(f"\nFetching prices for next 7, 14 and 30 days...")
    for days in DAYS_OUT:
        try:
            data, flight_date = fetch_flights(origin_sky, destination_sky, origin_entity, destination_entity, days)
            ...
            for item in itineraries:
                ...
                flights.append({"price": price, "airline": airline, "duration": duration, "stops": stops})
